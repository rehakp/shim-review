...
PATCHES=(
	...
	#AOS Patches
	"${FILESDIR}"/grub-add-extras.patch
	"${FILESDIR}"/grub-remove-welcome.patch
	"${FILESDIR}"/grub-eficonsole-togglekeys.patch
	"${FILESDIR}"/play.patch
	"${FILESDIR}"/grub-indicate-selected-menu-item-by-a-series-of-beeps.patch
	"${FILESDIR}"/menu-hotkey-add-space-alias.patch
	"${FILESDIR}"/support-animation.patch
	"${FILESDIR}"/readkey.patch
	"${FILESDIR}"/prevent-blinking-cursor.patch
	"${FILESDIR}"/menuentry-hidden-unfocusable-helptext-separator.patch
	"${FILESDIR}"/menu-visual-adjustments.patch
	"${FILESDIR}"/grub-print-powered.patch
	"${FILESDIR}"/grub-blind-debug.patch
	"${FILESDIR}"/grub-version.patch
	"${FILESDIR}"/store_current_user.patch
	"${FILESDIR}"/grub2-czech.patch
	"${FILESDIR}"/grub-sbat.patch
)
